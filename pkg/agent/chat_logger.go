...
func NewChatLogger(underlying gollm.Chat, store *store.DataStore) *ChatLogger {
	return &ChatLogger{
...
func (c *ChatLogger) writeRecord(record *gollm.RecordMessage) {
	if c.store == nil {
		return
	}
...
func (c *ChatLogger) Send(ctx context.Context, contents ...any) (gollm.ChatResponse, error) {
	c.writeRecord(&gollm.RecordMessage{
		Timestamp: time.Now(),
		Role:      "user",
		Content:   contents,
	})
...
// LoadHistory implements gollm.Chat.
func (c *ChatLogger) LoadHistory(history []*gollm.RecordMessage) error {
	return c.underlying.LoadHistory(history)
}
